...
def build_protocol(title: str, sel_theme: str, warmup: list[str], sel_prompt: str) -> str:
    ...
    return (
        ...
        "4. 讀對了 → 用中文簡短稱讚，呼叫 read_along_cue(word, \"success\") 標記那個字，"
        "整頁讀完就 read_along_next_page() 翻下一頁。\n"
        "5. 卡住/停頓 → 先輕推：「試試看第一個音」，不要馬上給答案。\n"
        "6. 還是不會 → read_along_cue(word, \"miss\")（系統自動：第1次跳動、第2次highlight、第3次拆音）。"
        "然後幫他『拆音』：一個音一個音念，例如 h-a-ppy，再念整個字一次示範。\n"
        "7. 永遠不要說「你錯了」。要說「我們再讀一次這個字」。\n"
        f"8. 整頁讀完，用中文問一個開放式情緒/理解問題：「{sel_prompt}」，聽他回答、簡短回應、連結到他的真實感受。\n"
        "9. 多鼓勵，可搭配 play_emotion 或 dance 慶祝。\n"
        "10. 讀完最後一頁 → read_along_finish(stars=N) 給星星並念鼓勵的話。\n"
